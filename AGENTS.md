...
- **平台实现**：新动作需在 `platforms/win32/` 和 `platforms/adb/` 中分别实现。
- **平台检测**：通过 `Controller.name` 和 `Controller.config` 属性检测平台类型（adb/win32），使用 `getattr()` 安全访问动态属性。
- **时间线模式**：复杂动作序列应使用时间线模式，支持并行动作叠加。
- **停止响应**：执行过程中需检查 `context.tasker.stopping`，收到停止通知时调用 `release_all()` 释放全部按键。
- **按键跟踪**：Win32 平台需维护 `_active_keys` 集合跟踪已按下未释放的按键，ADB 平台维护 `_active_contacts` 字典。
