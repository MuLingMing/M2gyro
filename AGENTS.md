...
时间线模式（`ActionTimeline`）采用优先级策略执行动作：

1. 先检查平台方法：`getattr(self._platform, action.action_name, None)`
2. 有平台方法：使用平台方法执行（保证时间准确、流畅）
   - 适用动作：`move` / `charge_attack` / `dodge` / `turn` / `interact` / `swipe` / `click` / `press_key` 等
3. 没有平台方法：使用 `action_registry.create_action()` 创建动作类并调用 `execute()`
   - 适用动作：`run_node` 等不需要平台实现的复杂动作
4. 瞬时动作：对于 `duration=0` 的瞬时动作，只在 `is_start=True` 时执行一次
...
2. 在 `platforms/desktop/desktop_platform.py` 和 `platforms/adb/adb_platform.py` 中实现
3. 创建动作类（`actions/basic/xxx_action.py`），继承 `ActionBase`，用 `@register_action` 注册
4. 在 `actions/basic/__init__.py` 中导入
5. 更新 Schema

##### 类型 B：不依赖平台的动作（类似 `run_node`）

1. 直接创建动作类（`actions/basic/xxx_action.py`），继承 `ActionBase`，用 `@register_action` 注册
