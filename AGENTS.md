...
- **动作定义**：新动作继承 `ActionBase`，使用 `@register_action` 注册。
- **平台实现**：新动作需在 `platforms/desktop/` 和 `platforms/adb/` 中分别实现。
- **平台检测**：通过 `Controller.name`/`Controller.config` 检测平台类型，使用 `getattr()` 安全访问。
- **平台命名**：桌面平台用 `desktop`（`.gitignore` 忽略 `win32/`）。
- **时间线模式**：复杂序列使用时间线模式，支持并行动作。区分依据：`operations` 中是否有 `duration`/`overlays` 字段。
- **统一参数格式**：时间线模式中 `duration`/`overlays` 放 `params` 内，顶层仅保留 `action` 和 `params`。
- **等待动作**：支持 `duration`（等待时长）和 `until`（目标时间点，仅时间线模式）。
- **停止响应**：检查 `context.tasker.stopping`，收到停止通知时调用 `release_all()` 释放全部按键。
- **按键跟踪**：Desktop 维护 `_active_keys`，ADB 维护 `_active_contacts`。
- **Schema 同步**：更新 `deps/tools/custom_act/OperationRecordAction.schema.json` 和主 Schema。

#### 时间线模式执行架构

优先级策略：① 平台方法 → ② 动作类 `execute()` → ③ 瞬时动作（`duration=0` 仅 `is_start=True` 时执行）

#### 新增动作流程

- **类型 A（依赖平台）**：`platforms/base.py` 抽象方法 → desktop/adb 实现 → 动作类注册 → `__init__.py` 导入 → Schema 更新
- **类型 B（不依赖平台）**：动作类注册 → `__init__.py` 导入 → Schema 更新
...
- **Skill 触发条件**：SKILL.md 的 `description` 准确、`name` 唯一、frontmatter 格式正确。
