...
class MeleeAttackAction(ActionBase):
    ...
    def execute(self, params: Dict[str, Any]) -> bool:
        ...
        执行流程：
        1. 获取蓄力时间参数（可选，未提供时使用平台默认值）
        2. 获取 X 坐标参数（可选）
        3. 获取 Y 坐标参数（可选）
        4. 调用平台 melee_attack 方法
        5. 返回执行结果
        """
        duration = params.get("duration")
        x = params.get("x")
        y = params.get("y")
        return self._platform.melee_attack(duration, x, y)
    ...
    def start(self, params: Dict[str, Any]) -> bool:
        ...
        x = params.get("x")
        y = params.get("y")
        return self._platform.melee_attack(0, x, y)
